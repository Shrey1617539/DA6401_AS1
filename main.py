...
sweep_configuration = {
    ...
    "parameters": {
        "number_of_epochs": {"values": [5]},
        "number_of_hidden_layers": {"values": [3]},
        "size_of_every_hidden_layer": {"values": [128]},
...
def main(args):
    ...
    weights, bias = T3.gradient_descent(X_train, y_train, weights, bias,  wandb.config.number_of_epochs, 
                                        activation_function= [wandb.config.activation_function for i in range(wandb.config.number_of_hidden_layers)], 
                                        learning_rate = wandb.config.learning_rate, batch_size = wandb.config.batch_size, 
                                        X_val = X_val, Y_val = y_val, X_test = X_test, Y_test = y_test, logging_train=True, logging_val=True)
